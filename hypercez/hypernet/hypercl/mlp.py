...
hyper_shapes_distilled` and the current statistics will be returned by the
        ...
        out_var (optional): If yes, will return a probabilistic output 
            of size [batchsize, 2*n_out]
    ...
    def __init__(self, n_in=1, n_out=1, hidden_layers=None,
                 ...
                 out_var=False, mlp_var_minmax=False):
        ...
        self._mlp_var_minmax = mlp_var_minmax
        if self._out_var:
            if self._mlp_var_minmax:
                self.mlp_max_logvar = nn.Parameter(torch.ones(n_out) / 2.0)
                self.mlp_min_logvar = nn.Parameter(-torch.ones(n_out) * 10.0)
            n_out = n_out * 2
        ...
        if out_var and (not mlp_var_minmax):
            shapes = [[1, n_out//2], [1, n_out//2]]
            self._param_shapes.extend(shapes)
            if init_weights is not None:
    ...
    def forward(self, x, weights=None, distilled_params=None, condition=None):
        ...
        if self._out_var:
            if self._mlp_var_minmax:
                max_logvar, min_logvar = self.mlp_max_logvar, self.mlp_min_logvar
            else:
                max_logvar, min_logvar = layer_weights[0], layer_weights[1]
        ...
        if self._out_var:
            mu, logvar = torch.split(hidden, hidden.size(-1)//2, dim=-1)
            logvar = max_logvar - torch.nn.functional.softplus(max_logvar - logvar)
            logvar = min_logvar + torch.nn.functional.softplus(logvar - min_logvar)

            hidden = torch.cat((mu, logvar), dim=-1)
        return hidden
