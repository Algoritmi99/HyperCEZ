...
def calc_fix_target_reg(hnet, task_id, targets=None, dTheta=None, dTembs=None,
    ...
    for i in ids_to_reg:
        ...
        elif si_omega is not None:
            _assert_shape_equality(weights_predicted, si_omega[i])
            SI = torch.cat([w.view(-1) for w in si_omega[i]])
            reg_i = (SI * (W_target - W_predicted).pow(2)).sum()
        else:
