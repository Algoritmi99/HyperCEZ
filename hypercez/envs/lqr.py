...
class LQR_2DCarGym(LQR_2DCar, Env):
    def __init__(self, noise=0.1, friction=0.1, mass=1.0, render_mode=None):
        super().__init__(noise=noise, friction=friction, mass=mass)
        self.render_mode = render_mode

        high = np.full((self.x_dim, 1), np.inf)
        self.observation_space = spaces.Box(low=-high, high=high, dtype=np.float32)
        self.action_space = spaces.Box(low=-10, high=10, shape=(2, 1), dtype=np.float32)
    ...
    def step(self, action):
        ...
        truncated = self.time_step >= 200  # optional time limit
...
class LQR_HARDGym(LQR_HARD, Env):
    def __init__(self, noise=0.1, friction=0.1, mass=1.0, render_mode=None):
        super().__init__(noise=noise, friction=friction, mass=mass)
        self.render_mode = render_mode
        ...
        high = np.full((self.x_dim, 1), np.inf)
        self.observation_space = spaces.Box(low=-high, high=high, dtype=np.float32)
        self.action_space = self.action_space  # Already defined
...
if __name__ == "__main__":
    env = LQR_2DCarGym(render_mode='human')
    env.reset()
    while True:
        env.render()
